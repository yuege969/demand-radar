...
const DEMO_PAIN_POINTS: PainPoint[] = [
  {
    ...
    title: "AI 法律文档自动审查工具",
    summary:
      "大量中小企业和自由职业者需要审查合同但请不起律师，希望有 AI 工具能自动识别风险条款并给出修改建议。",
    category: "AI",
    industry: "AI法律",
    pain_score: 8.2,
    keywords: '["AI", "法律", "合同审查"]',
    source_urls: null,
    is_saas_idea: true,
    is_plugin_idea: false,
    business_angle: "面向中小企业的 SaaS 订阅服务",
    source_count: 12,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
    is_individual_feasible: true,
    feasibility_reason: "核心基于 LLM API，单人可完成",
    estimated_dev_time: "4-8 周",
    tech_stack_hints: ["Next.js", "OpenAI API", "PostgreSQL"],
    market_saturation: "amber",
    individual_score: 7.5,
    opportunity_score: 78,
    demand_validation: null,
  ...
  },
  {
    ...
    title: "跨境电商选品数据分析平台",
    summary:
      "跨境电商卖家需要从多平台（Amazon、Shopify、TikTok Shop）聚合数据来分析选品趋势，现有工具价格高且数据不全面。",
    category: "电商",
    industry: "跨境电商",
    pain_score: 7.8,
    keywords: '["跨境电商", "选品", "数据分析"]',
    source_urls: null,
    is_saas_idea: true,
    is_plugin_idea: false,
    business_angle: "按月订阅的数据分析平台",
    source_count: 8,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
    is_individual_feasible: true,
    feasibility_reason: "爬虫 + 数据处理，独立开发者可完成 MVP",
    estimated_dev_time: "6-10 周",
    tech_stack_hints: ["Python", "React", "MongoDB"],
    market_saturation: "amber",
    individual_score: 6.8,
  ...
  {
    id: -3,
    title: "独立开发者日志与分析一体化工具",
    summary:
      "独立开发者需要同时管理多个项目的日志、错误追踪和用户行为分析，现有工具（Sentry、LogRocket）按量计费太贵。",
    category: "DevTools",
    industry: "独立开发者工具",
    pain_score: 8.5,
    keywords: '["DevTools", "日志", "监控"]',
    source_urls: null,
    is_saas_idea: true,
    is_plugin_idea: true,
    business_angle: "自托管开源 + 云服务双模式",
    source_count: 15,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
    is_individual_feasible: true,
    feasibility_reason: "开源社区已有类似方案可参考",
    estimated_dev_time: "8-12 周",
    tech_stack_hints: ["Rust", "React", "ClickHouse"],
    market_saturation: "green",
    individual_score: 6.0,
...
export default async function HomePage() {
  ...
  const statsResult =
    total > 0
      ? await fetchFromAPI<{ data: PainPoint[] }>(
          "/pain-points?per_page=1000&sort_by=opportunity_score"
        )
      : null;
  ...
  const stats = [
    ...
    {
      label: "SaaS 机会",
      value: String(allPoints.filter((p) => p.is_saas_idea).length),
      color: "text-amber-600",
    },
    {
      label: "可独立开发",
      value: String(allPoints.filter((p) => p.is_individual_feasible).length),
        ...
        <div className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {DEMO_PAIN_POINTS.map((pp) => (
              <div
                key={pp.id}
                className="rounded-xl border border-gray-200 bg-white p-5 opacity-80 hover:opacity-100 transition-opacity"
              >
